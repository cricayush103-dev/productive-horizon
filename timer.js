...
$("resetTimer").onclick=()=>{
  running=false;clearInterval(interval);elapsed=0;startedAt=null;
  pomoInitial=Number($("pomodoroStudy").value||25)*60;
  pomoRemaining=pomoInitial;
  render();
};
$("pomodoroStudy").onchange=()=>{
  if(!running){
    pomoInitial=Number($("pomodoroStudy").value||25)*60;
    pomoRemaining=pomoInitial;
    render();
...
$("timerType").onchange=()=>{
  running=false;clearInterval(interval);startedAt=null;elapsed=0;
  pomoInitial=Number($("pomodoroStudy").value||25)*60;
  pomoRemaining=pomoInitial;
  render();
};
...
async function saveSession(){
  ...
  const hasProgress = $("timerType").value==="Stopwatch" ? elapsed>0 : pomoRemaining<pomoInitial;
  if(!hasProgress)return alert("Run the timer first.");
  ...
  const {error}=await supabaseClient.from("study_sessions").insert({
    user_id:timerUser,
    subject_id,
    session_date:phLocalDate(),
    start_time:start.toISOString(),
    end_time:end.toISOString(),
    duration_minutes:duration,
    session_type:$("timerType").value,
    notes:$("timerNotes").value.trim()||null
  });

  if(error){
    console.error(error);
    return alert("Session could not be saved.");
  }
...
async function loadToday(){
  const {data,error}=await supabaseClient
    .from("study_sessions")
    .select("*")
    .eq("user_id",timerUser)
    .eq("session_date",phLocalDate())
    .order("created_at",{ascending:false});

  if(error){console.error(error);return;}

  const rows=data||[];
  const total=rows.reduce((a,r)=>a+Number(r.duration_minutes||0),0);
  ...
  $("sessionList").innerHTML=rows.length
    ? rows.map(r=>`<div class="module-row"><div class="module-row-main"><strong>${phEscape(subjects.find(s=>s.id===r.subject_id)?.name||"Subject")}</strong><span>${phEscape(r.session_type||"Study")} • ${r.duration_minutes||0} min${r.notes?` • ${phEscape(r.notes)}`:""}</span></div></div>`).join("")
    : `<div class="empty-mini">No sessions saved today.</div>`;
}
...
(async()=>{
  ...
  if(!s)return;
  timerUser=s.user.id;
  subjects=await phSubjects(timerUser);
